...
from moveit_msgs.msg import Constraints, PositionConstraint, OrientationConstraint, PlanningScene, CollisionObject, AttachedCollisionObject
...
PLANNING_TIME = 4.0        # More time to solve around the bubble
...
class UnchainedMover(Node):
    def __init__(self):
        ...
        time.sleep(1.0) # Wait for bubble to register
    ...
    def setup_safety_bubble(self):
        ...
        This forces MoveIt to keep the tool at least 4cm away from the body.
        The robot firmware limit is 2.8cm, so this guarantees safety.
        """
        scene_msg = PlanningScene()
        scene_msg.is_diff = True
        
        # Create the Virtual Shield
        shield = AttachedCollisionObject()
        shield.link_name = "tool0" # Attach to the wrist
        shield.object.header.frame_id = "tool0"
        shield.object.id = "safety_bubble"
        
        # Define Shape: A Cylinder surrounding the wrist
        # Radius 0.08m (8cm) ensures we stay far away from the arm
        primitive = SolidPrimitive()
        primitive.type = SolidPrimitive.CYLINDER
        primitive.dimensions = [0.15, 0.08] # [Height, Radius]
        
        shield.object.primitives.append(primitive)
        
        # Position it slightly "back" to cover the wrist joint
        pose = Pose()
    ...
    def find_and_execute_path(self, target_pose, move_id):
        ...
        if plan_result.error_code.val != 1:
            self.get_logger().info(f'[Move {move_id}] Unreachable/Collision (Code {plan_result.error_code.val}).')
            return False
